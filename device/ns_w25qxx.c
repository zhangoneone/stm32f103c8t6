...
int W25QXX_Init(void){	
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	return W25QXX_TYPE==W25QXX_ReadID();//读取FLASH ID.  
...
DSTATUS out_flash_write(BYTE pdrv, const BYTE* buff, DWORD sector, UINT count){
	W25QXX_Write(buff,sector*SECTOR_SIZE*8,count*SECTOR_SIZE);
	return RES_OK;
}
DSTATUS out_flash_ioctl(BYTE pdrv, BYTE cmd, void* buff){
	if(cmd == CTRL_SYNC){
	  return RES_OK;
	}
	else if(cmd == GET_SECTOR_COUNT){
		if(!buff)return RES_ERROR;
		*(UINT*)buff = SECTOR_NUM;
	}
	else if(cmd == GET_SECTOR_SIZE){
		if(!buff)return RES_ERROR;
		*(UINT*)buff = SECTOR_SIZE;
	}
	else if(cmd == GET_BLOCK_SIZE){
		if(!buff)return RES_ERROR;
		*(UINT*)buff = BLOCK_SIZE;
	}
...
const Out_Flash_t out_flash = {
	out_flash_initialize,
	out_flash_status,
	out_flash_read,
	out_flash_write,
	out_flash_ioctl,
